...
@unittest.skip("cvpods.export out-dated")
class TestCaffe2Export(unittest.TestCase):
    ...
    def _test_model(self, config_path, device="cpu"):
        ...
        from cvpods.export import Caffe2Model, add_export_config, export_caffe2_model

        cfg = model_zoo.get(config_path)
        ...
        DetectionCheckpointer(model).load(model_zoo.get_checkpoint_s3uri(config_path))
        c2_model = export_caffe2_model(cfg, model, copy.deepcopy(inputs))

        with tempfile.TemporaryDirectory(prefix="cvpods_unittest") as d:
    ...
    def _get_test_image(self):
        try:
            file_name = DatasetCatalog.get("coco_2017_train")[0]["file_name"]  # noqa
