...
from cvpods.structures import BoxMode, Instances, RotatedBoxes
...
class TestVisualizer(unittest.TestCase):
    ...
    def _random_data(self):
        ...
        polygons = [[_rand_poly() for _ in range(np.random.randint(1, 5))]
                    for _ in range(N)]
    ...
    @unittest.skip("`metadata=None` not supported")
    ...
    def test_draw_binary_mask(self):
        ...
        for m in [mask, mask_with_hole]:
            v = Visualizer(img, self.metadata)
            o = v.draw_binary_mask(m, color=[1, 0, 0], text=None, area_threshold=0)
